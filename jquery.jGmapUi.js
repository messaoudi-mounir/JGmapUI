   ...
   options = $.extend(defaults, options);      
    ...
    if(options.mapTypeControl){
		...
		for (var i = 0; i<gmapUi_maps.length; i++){
	  	  if(gmapUi_currMap == gmapUi_maps[i])
	  	   	checked="checked";
	  	  else 
	  	  	checked=""; 
	  	  	 
	  	   $('<input type="radio" onclik="" id="gmapUi_map_'+gmapUi_mapContainerId+i+'" name="map_'+gmapUi_mapContainerId+'" value="'+i+'"  '+checked+' /><label for="gmapUi_map_'+gmapUi_mapContainerId+i+'">' + gmapUi_maps[i].getName() + '</label>')
	  	   .change(function() {
	  	       var i = $(this).attr('value');
			   map.setMapType(gmapUi_map.getMapTypes()[i]);
		    })
	  	   .appendTo("#gmapUi_mapTypes_"+gmapUi_mapContainerId);
	    }
	    $("#gmapUi_mapTypes_"+gmapUi_mapContainerId).buttonset();
   }
